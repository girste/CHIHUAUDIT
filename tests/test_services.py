...
def test_analyze_services_with_risky_exposed():
    ...
    with patch("mcp_security.analyzers.services.parse_listening_ports", return_value=[
        {"port": 3306, "protocol": "tcp", "address": "0.0.0.0", "exposed": True, "process": "mysqld"},
        {"port": 5432, "protocol": "tcp", "address": "0.0.0.0", "exposed": True, "process": "postgres"},
    ]):
...
def test_analyze_services_with_failed_systemd():
    ...
    with patch("mcp_security.analyzers.services.parse_listening_ports", return_value=[
        {"port": 22, "protocol": "tcp", "address": "0.0.0.0", "exposed": True, "process": "sshd"},
    ]):
        with patch("mcp_security.analyzers.services.check_systemd_failed", return_value=[
            "nginx.service",
            "mysql.service",
...
def test_analyze_services_many_unknown():
    ...
    unknown_services = [
        {"port": 9000 + i, "protocol": "tcp", "address": "0.0.0.0", "exposed": True, "process": f"custom{i}"}
        for i in range(5)
    ]

    with patch("mcp_security.analyzers.services.parse_listening_ports", return_value=unknown_services):
        with patch("mcp_security.analyzers.services.check_systemd_failed", return_value=[]):
            with patch("mcp_security.analyzers.services.check_critical_services", return_value=[]):
                result = analyze_services()
...
def test_analyze_services_localhost_only():
    ...
    with patch("mcp_security.analyzers.services.parse_listening_ports", return_value=[
        {"port": 22, "protocol": "tcp", "address": "127.0.0.1", "exposed": False, "process": "sshd"},
        {"port": 80, "protocol": "tcp", "address": "127.0.0.1", "exposed": False, "process": "nginx"},
    ]):
        with patch("mcp_security.analyzers.services.check_systemd_failed", return_value=[]):
            with patch("mcp_security.analyzers.services.check_critical_services", return_value=[]):
                result = analyze_services()
...
def test_analyze_services_secure_config():
    ...
    with patch("mcp_security.analyzers.services.parse_listening_ports", return_value=[
        {"port": 22, "protocol": "tcp", "address": "0.0.0.0", "exposed": True, "process": "sshd"},
        {"port": 443, "protocol": "tcp", "address": "0.0.0.0", "exposed": True, "process": "nginx"},
    ]):
        with patch("mcp_security.analyzers.services.check_systemd_failed", return_value=[]):
            with patch("mcp_security.analyzers.services.check_critical_services", return_value=[
                {"name": "ssh", "status": "active", "active": True},
